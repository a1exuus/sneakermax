...
function initCatalogFilters() {
  ...
  function syncPriceValues() {
    if (!minPrice || !maxPrice || !minValue || !maxValue) return;
  ...
  function applyFilters() {
    ...
    products.forEach((product) => {
      ...
      const sizeMatches =
        ...
        (productSize && selectedSizes.has(productSize));
      const genderMatches =
        selectedGenders.size === 0 ||
        (productGender && selectedGenders.has(productGender));
  ...
  reset?.addEventListener("click", () => {
    document.querySelectorAll(".size.is-selected").forEach((button) => {
...
function initQuiz() {
  ...
  function validateCurrentStep() {
    ...
    if (currentStep === 0) {
      return step.querySelectorAll('input[name="type"]:checked').length > 0;
    }

    if (currentStep === 1) {
      return Boolean(step.querySelector('input[name="size"]:checked'));
    }
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (message) message.textContent = "Заявка готова к отправке.";
  });
...
function initFeedbackForm() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (message) {
      message.textContent = "Форма заполнена. Готово к отправке.";
    }
  });
}
